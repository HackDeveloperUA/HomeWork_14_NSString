...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    // Массив для предложений которые содержат "NSString"
    NSMutableArray* mutableArray    = [NSMutableArray new];
    
    // Массив для последних слов в предложениях которые содержат "NSString"
    NSMutableArray* lastWordsArray  = [NSMutableArray new];
    
    // Массив для первых слов в предложениях которые содержат "NSString"
    NSMutableArray* firstWordsArray = [NSMutableArray new];

    
    // Заполняем массив mutableArray предложениями
    for (NSString* str in array) {
        
        NSRange range = [str rangeOfString:@"NSString"];
        if (range.location != NSNotFound) {
            [mutableArray addObject:str];
        }
    }
    
    // Заполняем массив lastWordsArray
    for (NSString* str in mutableArray) {
        
        NSRange range    = [str rangeOfString:@" " options:NSBackwardsSearch];
        if (range.location != NSNotFound)
        {
            NSString *result = [str substringFromIndex:range.location+1];
            [lastWordsArray     addObject:result];
        }
    }
    
    // Заполняем массив firstWordsArray
    for (NSString* str in mutableArray) {
        ...
        if (range.location != NSNotFound)
        {
            ...
            [firstWordsArray     addObject:result];
        }
    }
    
    NSLog(@"Старый массив = %@n\n\n",mutableArray);

    // замена
    for (int i=0; i<[mutableArray count]; i++) {

    NSString* str    = [mutableArray objectAtIndex:i];
    // Меняем первое слово на последней
    str = [str stringByReplacingOccurrencesOfString:[firstWordsArray objectAtIndex:i] withString:[lastWordsArray  objectAtIndex:i]];
    // Т.К. у нас теперь первое и последнее слова одинаковы ,теперь нужно искать сзади последнее слово
    NSRange range = [str rangeOfString:[lastWordsArray  objectAtIndex:i] options:NSBackwardsSearch];
    str = [str stringByReplacingCharactersInRange:range withString:[firstWordsArray objectAtIndex:i]];
    [mutableArray setObject:str atIndexedSubscript:i];

    }
    
    NSLog(@"\n\n\n\n NEW ARRAY !!!! - %@ ",mutableArray);
    /*
     NSArray* array = [text componentsSeparatedByString:@". "];
     //NSLog(@"\n\n\n\n THIS IS ARRAY \n\n\n %@",array);
     ...
     NSMutableArray* mutableArray    = [NSMutableArray new];
     NSMutableArray* lastWordsArray = [NSMutableArray new];
     NSMutableString *firstWord = [NSMutableString new];
     ...
     // [firstWordsArray addObject:firstWord];
     
     // NSUInteger location = [str rangeOfString:@" "].location;
     // NSString *result = location == NSNotFound ? str : [str substringToIndex:location];
     //  [firstWordsArray addObject:result];
     ...
     NSLog(@"mutableArray  = %@n\n\n",mutableArray);
     NSLog(@"The array LAST WORDS %@",lastWordsArray);
